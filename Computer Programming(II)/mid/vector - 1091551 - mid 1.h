...
public:
	...
	vector& operator=(const vector& right)
	{
		...
		{
			size_type originalSize = size();
			/*if (right.size() > originalSize)    //deleted
			{*/
			...
			//} //deleted
			else if (right.size() <= originalSize) {
				myData.myEnd = &myData.myFirst[originalSize];
				value_type* j = myData.myFirst;
				for (int i = 0; i < right.size(); i++) {
					*j = right.myData.myFirst[i];
					j++;
		...
		if (newSize > originalSize)
		{
			if (newSize > capacity())
			{
				...
				if (newCapacity < newSize)
					newCapacity = newSize;
				value_type* j = new value_type[newCapacity]();
				myData.myEnd = &j[newCapacity];
				int i = 0; //added
				while (i < originalSize) { 				
					j[i] = myData.myFirst[i];
					i++;
				}
				myData.myFirst = &j[0];
				myData.myLast = &j[newSize];
				delete[]tempPtr;
			...
			myData.myLast = &myData.myFirst[newSize];
			//myData.myEnd = &myData.myFirst[originalSize];  //deleted
		}
	}
