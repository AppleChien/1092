...
public:
	...
	iterator insert(const_iterator where, const Ty& val) // insert val at where
	{
		...
		nodePtr old_position = where->prev->next;

		newNode->next = where->prev->next;
	...
	iterator erase(const_iterator where)
	{
		...
		nodePtr old_position = where->next;
...
bool operator==(const list< Ty >& left, const list< Ty >& right)
{
	if (left.size != right.size)
		...
		typename Ty::nodePtr left_ptr = left.myData.myHead;
		typename Ty::nodePtr right_ptr = right.myData.myHead;
		for (; left_ptr != left.myData.myHead->prev; left_ptr = left_ptr->next, right_ptr = right_ptr->next) {
