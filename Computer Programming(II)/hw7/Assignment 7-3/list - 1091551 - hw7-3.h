...
public:
	...
	list& operator=(const list& right)
	{
		...
		{
			if (size() < right.size()) {
				nodePtr i = myData.myHead->next;//begin()
				nodePtr j = right.myData.myHead->next;

				int a = 0;
				for (a; a < size(); a++, i = i->next, j = j->next) {
					i->myVal = j->myVal;
				}
				for (a; a < right.size(); a++, j = j->next) {
					insert(cend(), j->myVal);
				}
			}
				...
				int a = 0;
				for (a; a < right.size(); a++, i = i->next, j = j->next) {
					i->myVal = j->myVal;
				}
